...
import { StyleSheet, Text, View, Dimensions, TouchableOpacity, ScrollView, RefreshControl, FlatList, Image, Platform, BackHandler, Alert } from 'react-native';
import { StackNavigationProp } from '@react-navigation/stack';
import { BookingStackParamList } from '../stacks/Navigator';
import { roomSlots } from '../slots/roomSlots';
import { roomPictures } from '../slots/roomPictures';
import moment from 'moment';
import { usingTimeSlots } from '../slots/usingTimeSlots';
...
interface BookingScreenProps {
  navigation: StackNavigationProp<BookingStackParamList, 'Booking'>; // 네비게이션 속성에 대한 타입으로 방금 지정해주었던 MainScreenNavigationProps 을 지정
};
...
const BookingScreen: React.FunctionComponent<BookingScreenProps> = (props) => {
...
const handleReserveDaytime = () => {
  // Calculate the end time based on the selected duration
    ...
    navigation.navigate('BookedInfo', {
        selectedUsingTimeSlot,
        selectedDateSlot,
        isMorning: true,
        isEvening: false,
...
<View style={{justifyContent:'center',alignItems:'center',right:'8%'}}>
    ...
    {chunkArray(availableNightTimeSlots, 4).map((row, rowIndex) => (
        ...
        <View style={styles.usingTime}>
            ...
            <View style={{justifyContent:'center',alignItems:'flex-start',height:screenWidth*0.25,marginLeft:'4%',backgroundColor:'white'}}>
                    <Text style={styles.usingTimeTitle}>결제금액: {price.toLocaleString()}원</Text>
            </View>


            <View style={{justifyContent:'center',alignItems:'center',backgroundColor:'white',height:screenWidth*0.2}}>
                    <TouchableOpacity
